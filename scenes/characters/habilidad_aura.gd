...
func _process(delta):
	...
	if activa:
		...
		# Notificar al padre (Fantasma) para que chequee plataformas
		if get_parent().has_method("_actualizar_proximidad_plataformas"):
			get_parent()._actualizar_proximidad_plataformas(radio_actual)

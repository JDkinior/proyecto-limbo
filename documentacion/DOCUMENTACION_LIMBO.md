...
# Operación de multijugador EOS P2P

## Qué cambió

El modo Online ahora usa el socket `limbop2pv1`, espera a que EOS Connect entregue un Product User ID antes de crear el peer y publica salas con atributos de protocolo y nombre. Al unirse, el cliente debe entrar primero al lobby y el host solo acepta solicitudes P2P de miembros de ese lobby.

Las conexiones Online tienen un límite de 35 segundos. En NAT abierto o moderado se intenta la ruta directa y, si falla, se repite con `ForceRelays`. En NAT estricto, móvil, VPN o desconocido, se usa Epic Relay desde el primer intento. No es necesario abrir puertos ni configurar UPnP: eso no arregla CGNAT ni datos móviles.

## Configuración obligatoria en Epic Developer Portal

Esta configuración no puede corregirse desde el cliente. Antes de probar, confirma lo siguiente en el mismo producto de EOS que usan el editor y el APK:

1. En **Product Settings > Clients**, el `Client ID` activo debe tener una política de tipo **Peer2Peer**. Si se usa una política personalizada, debe permitir EOS Connect, Lobby y P2P; no habilites permisos que el juego no use.
2. Los dos ejecutables deben usar exactamente el mismo `Product ID`, `Sandbox ID`, `Deployment ID` y `Client ID`. Una mezcla entre credenciales de staging y producción no podrá verse ni conectar.
3. Para Android, el paquete debe incluir el SDK Android de EOS y las tres permisiones `INTERNET`, `ACCESS_NETWORK_STATE` y `ACCESS_WIFI_STATE`. La plantilla local ya contiene esta configuración y el proyecto exporta con Gradle y `arm64-v8a`.
4. Rota el `Client Secret` que estaba escrito en `scripts/core/eos_manager.gd` antes de distribuir o subir el repositorio. Mantén credenciales de desarrollo en `.env` (ahora ignorado por Git) y aplica el principio de privilegios mínimos a la Client Policy.

## Prueba de aceptación recomendada

1. Exporta e instala el mismo build actual en Android; los APK antiguos no ven las salas del nuevo protocolo.
2. Inicia una partida Online en Windows y verifica el mensaje `Sesión EOS lista` antes de crear la sala.
3. En Android, con datos móviles o una Wi-Fi distinta, refresca salas, entra a la sala y espera la pantalla de lobby.
4. En el log busca `Transporte EOS establecido por Epic Relay` o `ruta directa`. En datos móviles, `Epic Relay` es el resultado esperado.
5. Repite invirtiendo host y cliente, y una vez con ambos dispositivos en la misma Wi-Fi. Ninguna de las tres pruebas debe depender de redirección de puertos del router.

Si la sala se crea pero el log muestra `EOS no pudo iniciar sesión`, `No se pudo abrir el servidor EOS P2P` o `Solicitud P2P rechazada`, conserva el log completo y revisa primero las credenciales y la Client Policy. Si no aparece ninguna sala, ambos dispositivos no están apuntando al mismo sandbox/deployment o el host no consiguió publicar el lobby.


---


## 🚨 ADVERTENCIAS CRÍTICAS DE RED Y MULTIJUGADOR (¡No romper!) 🚨

### 1. EOS P2P y Epic Developer Portal
- El portal de Epic Games está configurado para habilitar automáticamente los servidores TURN (Relays) cuando se utiliza el tipo de política de cliente **Peer2Peer**. 
- Si la conexión online falla en redes móviles (CGNAT) con `Reason 7` o se queda colgada y da `Reason 3/Reason 2`, y sabes que tu código está bien, el problema **siempre será de infraestructura**. Revisa que el portal tenga los permisos habilitados y que la red móvil no bloquee los puertos UDP.
- **Nunca uses `ForceRelays` (Forzar Relays) en la misma red LAN para probar**, porque Epic rechazará la solicitud de relay (Hairpinning) y fallará instantáneamente con `Reason 7`. El código actual usa `AllowRelays` en el primer intento para permitir la conexión en la misma casa, y hace fallback a `ForceRelays` en el segundo intento. **Esta es la arquitectura definitiva, no la modifiques.**
- El host acepta las conexiones automáticamente en `_on_eos_incoming_connection_request` al momento (`eos_peer.accept_connection_request(remote_puid)`). No agregues retrasos ni comprobaciones asíncronas ahí porque romperán el flujo del SDK y causarán Timeouts.

### 2. Windows Firewall y Pruebas Locales (Zombis)
- Si intentas hostear desde PC y conectar desde un celular por Wi-Fi (LAN), **Windows Firewall bloqueará los paquetes UDP entrantes** y el celular no conectará. Tienes que apagar Windows Firewall o darle permiso explícito a Godot.
- Sin embargo, abrir 2 instancias de Godot en el **mismo PC** sí funciona porque usan el Localhost, el cual ignora el Firewall.
- Si en medio de las pruebas de red cierras la ventana bruscamente, el proceso `Godot` se puede quedar abierto en segundo plano (Zombi) ocupando el puerto de red e interceptando los paquetes. Si de repente el juego local deja de funcionar, revisa el Administrador de Tareas y mata el proceso oculto de Godot.

### 3. Conexiones Fantasmas (VPNs)
- Si cambias de red bruscamente o apagas una VPN durante el juego, el Host tardará **45 segundos** en darse cuenta de que la conexión murió. Si intentas reconectar antes de esos 45 segundos, te quedarás cargando en "Conectando...". Reinicia la sala para limpiar las conexiones fantasma al instante.
